...
/**
 * D2 简化版 5 维自适应融合定位器 —— 外接加速度计 + Pinpoint + Limelight + EKF5D。
 *
 * <p>相对 {@link AdaptiveEKF5DLocalizer}（D3 模式，3D 斜坡补偿里程计）的简化版本，固定采用
 * <b>标准 2D 里程计</b>（{@link PinpointLocalizer}），核心差异与数据角色如下：
 * <ol>
 *   <li><b>外接加速度计</b>（{@link Rev9axisIMU}, BNO055）测出体坐标系线性加速度，结合 Hub IMU
 *       pitch/roll 姿态角<b>旋转后作为控制输入</b>驱动 {@link EKF5D} 预测；<b>过程噪声 Q 保持固定</b>
 *       （沿用 {@link EKF5D} 默认 Q，不做自适应）；</li>
 *   <li><b>Pinpoint</b> 提供体坐标系水平速度与航向（观测），观测噪声 {@code R_pin} 自适应，
 *       与 {@link AdaptiveEKF5DLocalizer} 相比<b>仅用角度判定</b>：
 *       <ul>
 *         <li>X（前后速度）由 <b>pitch 倾角</b>大小判定；</li>
 *         <li>Y（左右速度）由 <b>roll 倾角</b>大小判定；</li>
 *         <li>θ（航向）仍由 <b>yaw 角加速度</b>（旋转冲击）判定，与 {@link AdaptiveEKF5DLocalizer} 一致。</li>
 *       </ul></li>
 *   <li><b>Limelight</b>（{@link MT1Localizer}）提供低频绝对位置观测，观测噪声 {@code R_vision}
 *       自适应逻辑与 {@link AdaptiveEKF5DLocalizer} 完全一致（MT1 stdDev → R 映射）。</li>
 * </ol>
 */
@Config
public class AdaptiveEKF5DLocalizer_D2 implements Localizer {
    ...
    // ---- R 自适应: MT1 stdDev (与 AdaptiveEKF5DLocalizer 一致) ----
     ...
     */
    public AdaptiveEKF5DLocalizer_D2(HardwareMap hardwareMap, Limelight3A limelight,
        ...
        // 外置加速度计 (与 AdaptiveEKF5DLocalizer D3 相同安装朝向约定)
    ...
    public PoseVelocity2d update() {
        ...
        ekf.setOdomR(adaptOdomR(dt, pitch, roll));

        // ---- 5. EKF5D 预测 (Q 固定, 加速度旋转由 pitch/roll 完成) ----
        ...
        // ---- 8. MT1 视觉 → 自适应 R (与 AdaptiveEKF5DLocalizer 一致) + 视觉更新 ----
        mt1.update();
        if (mt1.isValid()) {
    ...
    // ==================== 自适应 R_vision (与 AdaptiveEKF5DLocalizer 一致) ====================
